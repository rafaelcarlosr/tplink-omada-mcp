...
describe('registerAllTools category filtering', () => {
    ...
    it('registers only tools matching active categories and permissions', () => {
        ...
        // dashboard read tools are a subset of all 336 tools
        expect((mockServer.registerTool as ReturnType<typeof vi.fn>).mock.calls.length).toBeLessThan(336);
    });

    it('registers all 336 tools when no activeCategories provided', () => {
        registerAllTools(mockServer, mockClient);
        expect(mockServer.registerTool).toHaveBeenCalledTimes(336);
    });
